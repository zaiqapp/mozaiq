...
export default function GaugeWidget({ config }: WidgetProps) {
  ...
  return (
    <div className="flex h-full flex-col items-center justify-center p-4">
      <p className="mb-1 text-sm font-medium text-[#e5e7eb]">{c.title}</p>
      <div className="relative h-32 w-32">
        <ResponsiveContainer width="100%" height="100%">
          ...
          >
            <RadialBar dataKey="value" cornerRadius={4} background={{ fill: 'rgba(255,255,255,0.1)' }} />
          </RadialBarChart>
        </ResponsiveContainer>
        <div className="absolute inset-0 flex items-center justify-center pt-6">
          <span className="text-xl font-bold text-[#f9fafb]">{c.value}{c.unit}</span>
        </div>
      </div>
    </div>
  )
